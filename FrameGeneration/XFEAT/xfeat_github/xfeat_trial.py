...
if __name__ == "__main__":
    ...
    # --- Read Input Video ---
    try:
        frame_gen_instance, shape, fps, length = read_video_frames(args.input_video) # Note: renamed for clarity
    ...
    print("\nStarting interpolation and writing process...")
    ...
    def generate_output_frames(frame_generator_instance): # <--- Accept instance here
        try:
            # Use the passed generator instance directly
            previous_frame = next(frame_generator_instance)
        except StopIteration:
            print("Input video was empty.")
            return # No frames to process
        except Exception as e:
            print(f"Error getting first frame: {e}")
            return

        yield previous_frame # Yield the first original frame

        previous_features = None
        # Compute features for the first frame
        with torch.no_grad():
             previous_features = xfeat_model.detectAndCompute(numpy_to_torch(previous_frame, device), top_k=args.max_kpts)[0]

        frame_index = 0 # Index of the *previous* frame

        for current_frame in frame_generator_instance:
            # print(f"\n--- Processing original frame pair ending at {frame_index+2}/{length} ---")

            # --- Detect & Describe current frame ---
            # start_feat_time = time.time() # Optional timing
            with torch.no_grad():
                current_features = xfeat_model.detectAndCompute(numpy_to_torch(current_frame, device), top_k=args.max_kpts)[0]
            # print(f"  Feature detection time: {time.time() - start_feat_time:.3f}s")

            # --- Match and Interpolate ---
            if previous_features is not None and current_features is not None:
                # print(f"  Matching features between frame {frame_index+1} and {frame_index+2}...")
                # start_match_time = time.time() # Optional timing
                motion_features = get_xfeat_motion_features(
                    xfeat_model, previous_features, current_features, args.match_thresh
                )
                # print(f"  Matching time: {time.time() - start_match_time:.3f}s ({len(motion_features)} matches)")
                ...
                # Fallback: Simple blend if features missing for either frame
                print(f"  Warning: Missing features for frame pair ending at {frame_index+2}. Blending.")
                yield cv.addWeighted(previous_frame, 0.5, current_frame, 0.5, 0.0)

            yield current_frame # Yield the current original frame
        ...
        # --- Generator function ends here ---

    # --- Write Output Video ---
    try:
        # Calculate estimated total frames correctly
        estimated_total_frames = max(0, 2 * length - 1) if length > 0 else 0

        # Pass the generator *instance* to the function that uses it
        output_frame_gen_instance = generate_output_frames(frame_gen_instance)

        # Pass the new generator instance and the calculated count to the writer
        write_video_frames(args.output_video, shape, output_frame_gen_instance, fps * 2, estimated_total_frames)
