...
def parse_arguments():
    ...
    parser.add_argument('--training_type', type=str, default='xfeat_default',
                        choices=['xfeat_default', 'xfeat_synthetic', 'xfeat_megadepth', 'xfeat_joint_kp'],
...
class Trainer():
    ...
    def __init__(self, megadepth_root_path, 
        ...
        self.net = XFeatModel(joint_kp = (model_name == 'xfeat_joint_kp')).to(self.dev)
        ...
        ##################### Synthetic COCO INIT ##########################
        if model_name in ('xfeat_default', 'xfeat_synthetic', 'xfeat_joint_kp'):
        ...
        ##################### MEGADEPTH INIT ##########################
        if model_name in ('xfeat_default', 'xfeat_megadepth', 'xfeat_joint_kp'):
    ...
    def train(self):
        ...
        for i in pbar:
            ...
            # new forward pass for joint
            if self.model_name == 'xfeat_joint_kp':
            ...
            for b in range(len(positives_c)):
                ...
                # loss_items.append(loss_kp_pos.unsqueeze(0)) # commented for joint training
                ...
                if self.model_name != 'xfeat_joint_kp':
                    ...
                    acc_pos = torch.tensor([0.0], device=self.dev)
            ...
            if self.model_name != 'xfeat_joint_kp':
                ...
                pbar.set_description( 'Loss: {:.4f} acc_c0 {:.3f} acc_c1 {:.3f} acc_f: {:.3f} loss_c: {:.3f} loss_f: {:.3f} loss_kp: {:.3f} #matches_c: {:d}'.format( loss.item(), acc_coarse_0, acc_coarse, acc_coords, loss_coarse, loss_coord, loss_l1, nb_coarse) )
            ...
            # self.writer.add_scalar('Accuracy/kp_position', acc_pos, i) #commented for joint training
            ...
            # self.writer.add_scalar('Loss/keypoint_pos', loss_kp_pos, i) #commented for joint training
...
if __name__ == '__main__':
    ...
    trainer = Trainer(
        ...
        # joint_kp=True if args.training_type == 'xfeat_joint_kp' else False
    )
