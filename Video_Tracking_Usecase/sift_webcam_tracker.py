...
import sys
import os # Keep os import in case it's needed elsewhere, though not for XFeat now

# --- Configuration ---
WEBCAM_INDEX = 0          # Index of the webcam (usually 0, 1, etc.)
RESIZE_SCALE_FACTOR = 0.6 # Adjust for performance vs detail
# SIFT Parameters
SIFT_NFEATURES = 0 # Unlimited features (or set a specific number like 2048)
SIFT_CONTRAST_THRESH = 0.04
SIFT_EDGE_THRESH = 10
# Matching Parameters
MATCHER_RATIO_THRESH = 0.75 # Lowe's ratio test threshold (lower is stricter)
...
OUTPUT_WINDOW_NAME = 'SIFT Enhanced Visual Odometry (Webcam)'
...
# --- SIFT and Matcher Initialization ---
print("Initializing SIFT detector...")
try:
    # Parameters: nfeatures, nOctaveLayers, contrastThreshold, edgeThreshold, sigma
    ...
    print("SIFT detector initialized.")
     ...
     print("Error: Your OpenCV installation might not include SIFT.")
     print("You might need to install 'opencv-contrib-python' instead of 'opencv-python'.")
     print("Try: pip uninstall opencv-python")
     print("Then: pip install opencv-contrib-python")
     exit()
except Exception as e:
     print(f"An unexpected error occurred initializing SIFT: {e}")
     exit()


print("Initializing BFMatcher...")
# BFMatcher with default params (NORM_L2 is good for SIFT)
matcher = cv2.BFMatcher()
print("BFMatcher initialized.")
# --- End SIFT Initialization ---
...
while True:
    ...
    des2 = None    # Descriptors (NumPy)
    kp2_coords = None # Coordinates (NumPy)
    num_kp2 = 0
    try:
        # Detect SIFT keypoints and compute descriptors
        kp2_obj, des2 = sift.detectAndCompute(gray_frame, None)
        ...
        if kp2_obj is not None and len(kp2_obj) > 0:
            ...
            kp2_obj = [] # Ensure it's an empty list if none found
            num_kp2 = 0

    except Exception as e:
        print(f"  Warn: SIFT detectAndCompute failed on frame {frame_count}: {e}")
        kp2_obj = []
        des2 = None
        kp2_coords = None
        num_kp2 = 0
    ...
    cv2.putText(panel_features, "Detected Keypoints (SIFT)", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, CLR_TEXT, 1, cv2.LINE_AA)
    ...
    if old_frame_data['des'] is not None and des2 is not None and \
       len(old_frame_data['des']) > 0 and len(des2) > 0:
        ...
        try:
            # Find k=2 best matches for each descriptor in 'old_des'
            ...
            if num_raw_matches >= MIN_MATCH_COUNT:
                ...
                mkpts2_np = np.float32([kp2_coords[m.trainIdx] for m in good_matches]) # Use current frame's coords
                ...
                if H is not None and mask is not None:
                    ...
                    if num_inliers >= MIN_MATCH_COUNT:
                        ...
                        # Use the *inlier* points derived from RANSAC mask
             ...
             # Catch potential errors during matching or homography (e.g., assertion failures)
    ...
    cv2.putText(panel_stats, "Motion & Stats (SIFT)", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, CLR_TEXT, 1, cv2.LINE_AA)
    ...
    info_text = [
        f"Frame: {frame_count}",
        f"Kpts: {num_kp2}",
        f"Matches (Ratio Test): {num_raw_matches}", # Clarify it's post-ratio test
    ...
    # Store data from the *current* frame to be used in the *next* iteration
    ...
    else: # Reset if detection failed
         old_frame_data = {'kp_obj': None, 'des': None, 'kp_coords': None}
